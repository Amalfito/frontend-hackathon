...
begin
  ...
  update public.game_state set
    ...
    -- endgame remis à zéro + le chrono learn disparaît (la bombe prend le relais)
...
-- --- Chrono « mode apprentissage » (cf. 0004_learn_timer.sql) ---------------
create or replace function public.learn_timer_start(p_admin_id uuid, p_minutes int)
returns jsonb language plpgsql security definer set search_path = public, extensions as $$
begin
  if not public.is_admin(p_admin_id) then return jsonb_build_object('error','forbidden'); end if;
  update public.game_state set
    learn_status='running', learn_duration_seconds=greatest(1,p_minutes)*60,
    learn_started_at=now(), learn_ends_at=now()+make_interval(mins => greatest(1,p_minutes)),
